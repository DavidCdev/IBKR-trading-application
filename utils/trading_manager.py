...
class TradingManager:
        ...
        try:
            ...
            with self._config_lock:
                ...
                try:
                    ...
                    symbol_changed = (
                            incoming_symbol is not None and str(incoming_symbol) != str(
                        ...
                        try:
                            with self._position_lock:
                                    ...
                                    try:
                                        ...
                                        matches_new_underlying = (
                                                ...
                                                (isinstance(pos_symbol, str) and (
                                                        pos_symbol == self.underlying_symbol or pos_symbol.startswith(
                                ...
                                if symbols_to_remove:
                                    logger.info(
                                        f"Removed {len(symbols_to_remove)} positions not matching new underlying '{self.underlying_symbol}'")
    ...
    def _calculate_order_quantity(self, option_price: float) -> int:
        ...
        try:
            ...
            if option_price > 0:
                ...
                logger.info(
                    f"Order quantity calculation: GUI={gui_max_value}, Tiered={tiered_max_value}, PDT={pdt_max_value}, Final={max_trade_value}, CostPerContract=${cost_per_contract:.2f}, Qty={quantity}")
            ...
            for risk_level in risk_levels_snapshot:
                ...
                if current_pnl_percent >= loss_threshold:
                    ...
                    if self._account_value > 0:
                        ...
                        logger.info(
                            f"Tiered risk limit: PnL={current_pnl_percent}%, Threshold={loss_threshold}%, Limit={account_trade_limit}%, MaxValue=${max_trade_value:.2f}")
    ...
    def _calculate_pdt_buffer(self) -> float:
        ...
        try:
            ...
            logger.info(
                f"PDT buffer calculation ({currency_upper}): Account=${self._account_value:.2f}, Min=${pdt_minimum}, Available=${available_buffer:.2f}, Safe=${safe_buffer:.2f}")
    ...
    async def place_buy_order(self, option_type: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Risk validation passed: Quantity={quantity}, Trade Value=${actual_trade_value:.2f} (${cost_per_contract:.2f} per contract)")
    ...
    async def place_sell_order(self, use_chase_logic: bool = True) -> bool:
        ...
        try:
                ...
                for pos in self._active_positions.values():
                    try:
                        ...
                        if contract_symbol:
                            ...
                            matches_underlying = (
                                    pos_symbol_str == self.underlying_symbol or pos_symbol_str.startswith(
                                f"{self.underlying_symbol} ")
                            )
            ...
            logger.info(
                f"Processing sell order for position: {symbol}, quantity: {quantity}, type: {position.get('position_type', 'UNKNOWN')}")
            ...
            if position.get('pnl_percent', 0) > 0 and self.runner > 0:
                ...
                logger.info(
                    f"Runner logic: Selling {sell_quantity} of {quantity} contracts (keeping {self.runner} as runner)")
            else:
            ...
            if use_chase_logic:
                ...
                with self._order_lock:
                    ...
                    logger.info(
                        f"Chase order {trade.order.orderId} initialized with contract: {getattr(contract, 'symbol', 'N/A')}, quantity: {sell_quantity}")
    ...
    def handle_order_status_update(self, order_id: int, status: str, filled: float = 0, remaining: float = 0,
                                   avg_fill_price: float = 0):
        """Handle order status updates from IB callbacks and clean up chase monitoring"""
        try:
            logger.info(
                f"Order status update: Order {order_id}, Status: {status}, Filled: {filled}, Remaining: {remaining}")
                ...
                with self._order_lock:
                    ...
                    for order_id, chase_data in list(self._chase_orders.items()):
                        ...
                        if time_elapsed >= 10:  # 10 seconds
                            ...
                            logger.info(
                                f"Chase order {order_id} ready for conversion to market: {time_elapsed:.1f}s elapsed")
    ...
    async def _convert_to_market_order(self, order_id: int):
        ...
        try:
            ...
            contract = self._ensure_contract_routable(contract)
            logger.info(
                f"Contract for market order conversion: {contract}, symbol: {getattr(contract, 'symbol', 'N/A')}")

            # Submit the market order to IB
            try:
                trade = self.ib.placeOrder(contract, market_order)
                logger.info(
                    f"Market order submitted to IB: {remaining_quantity} contracts, order ID: {trade.order.orderId}")
            except Exception as submit_error:
                logger.error(f"Failed to submit market order to IB: {submit_error}")
                self._last_action_message = f"Failed to submit market order: {str(submit_error)}"
                return
    ...
    async def _place_bracket_orders(self, parent_order_id: int, contract, quantity: int, entry_price: float,
                                    option_type: str):
        ...
        try:
            ...
            logger.info(
                f"Placing bracket orders for {option_type}: Stop Loss={stop_loss_percent}%, Profit Gain={profit_gain_percent}%")
            ...
            if profit_gain_percent:
                ...
                take_profit_price = validate_and_round_price(raw_take_profit_price,
                                                             f"Take profit order for {option_type}")
    ...
    def _calculate_stop_loss_price(entry_price: float, stop_loss_percent: float) -> float:
        ...
        try:
            ...
            logger.info(
                f"Stop loss calculation: Entry=${entry_price:.2f}, Loss={stop_loss_percent}%, Stop=${stop_loss_price:.2f}")
    ...
    def _calculate_take_profit_price(entry_price: float, profit_gain_percent: float) -> float:
        ...
        try:
            ...
            logger.info(
                f"Take profit calculation: Entry=${entry_price:.2f}, Gain={profit_gain_percent}%, Target=${take_profit_price:.2f}")
        ...
        try:
            ...
            with self._position_lock:
                if symbol in self._active_positions:
                    ...
                    logger.info(
                        f"Position confirmed as filled: {symbol}, {filled_quantity} contracts at ${fill_price:.2f}")
                else:
                    ...
                    logger.info(
                        f"Position created from fill (fallback): {symbol}, {filled_quantity} contracts at ${fill_price:.2f}")
                ...
                if order_id in self._open_orders:
                    ...
                    if order_data['type'] == 'BUY':
                        ...
                        if order_id in self._chase_orders:
                            ...
                            logger.info(
                                f"Updated chase order {order_id} remaining quantity: {old_quantity} -> {remaining_quantity}")
                            if remaining_quantity <= 0:
                                removed_data = self._chase_orders.pop(order_id)
            ...
            with self._position_lock:
                if symbol in self._active_positions:
                    ...
                    if new_size <= 0:
                        ...
                        logger.info(
                            f"Position fully closed: {symbol}, {filled_quantity} contracts sold at ${fill_price:.2f}")
                    else:
                        # Partial position closed
                        position['position_size'] = new_size
                        logger.info(
                            f"Partial position closed: {symbol}, {filled_quantity} contracts sold at ${fill_price:.2f}, remaining: {new_size}")
                else:
    ...
    def calculate_max_affordable_quantity(self, option_price: float) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Max affordable quantity calculation: {max_quantity} contracts at ${option_price:.2f} (limited by {limiting_constraint})")
            return result
    ...
    def validate_option_price_for_ib(self, price: float, context: str = "") -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Price validation for {context}: ${price:.4f} -> {'VALID' if result['is_valid'] else 'INVALID'} (tick size: ${result['tick_size']:.2f})")
            return result
    ...
    def analyze_tick_size_compliance(self, prices: List[float], context: str = "") -> Dict[str, Any]:
        ...
        try:
            ...
            analysis['summary'] = {
                ...
                'max_adjustment_amount': max([p['adjustment_amount'] for p in analysis['price_details']]) if analysis[
                    'price_details'] else 0,
                'avg_adjustment_amount': sum([p['adjustment_amount'] for p in analysis['price_details']]) / len(
                    analysis['price_details']) if analysis['price_details'] else 0
            }

            # Generate recommendations
            if analysis['non_compliant_prices'] > 0:
                analysis['recommendations'].append(
                    f"Fix {analysis['non_compliant_prices']} non-compliant prices to prevent IBKR Error 110")
                analysis['recommendations'].append("Use the rounded prices provided in price_details")
                analysis['recommendations'].append(
                    "Consider implementing automatic tick size validation in order placement")
            ...
            logger.info(
                f"Tick size compliance analysis for {context}: {analysis['compliant_prices']}/{len(prices)} prices compliant")
            return analysis
